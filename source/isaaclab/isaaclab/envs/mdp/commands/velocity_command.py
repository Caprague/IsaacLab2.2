...
if TYPE_CHECKING:
    ...
    from .commands_cfg import NormalVelocityCommandCfg, UniformVelocityCommandCfg, UniformVelocityCommandCfgUser
...
class UniformVelocityCommandUser(CommandTerm):
    r"""Command generator that generates a velocity command in SE(2) from uniform distribution.
    ...
    cfg: UniformVelocityCommandCfgUser
    """The configuration of the command generator."""

    def __init__(self, cfg: UniformVelocityCommandCfgUser, env: ManagerBasedEnv):
        """Initialize the command generator.
        ...
        self.vel_command_b = torch.zeros(self.num_envs, 3, device=self.device)
        self.heading_target = torch.zeros(self.num_envs, device=self.device)
        self.is_vel_world_env = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
    ...
    def __str__(self) -> str:
        ...
        msg += f"\tCommand dimension: {tuple(self.command.shape[1:])}\n"
        msg += f"\tResampling time range: {self.cfg.resampling_time_range}\n"
        msg += f"\tWorld velocity probability: {self.cfg.rel_vel_world_envs}"
        msg += f"\tStanding probability: {self.cfg.rel_standing_envs}"
        return msg
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        self.vel_command_b[env_ids, 1] = r.uniform_(*self.cfg.ranges.lin_vel_y)
        # -- ang vel yaw - rotation around z
        self.vel_command_b[env_ids, 2] = torch.zeros_like(r)
        ...
        # -- ang vel yaw - rotation around z
        self.vel_command_w[env_ids, 2] = torch.zeros_like(r)

        # heading target
        self.heading_target[env_ids] = r.uniform_(*self.cfg.ranges.heading)

        # update world frame envs
        self.is_vel_world_env[env_ids] = r.uniform_(0.0, 1.0) <= self.cfg.rel_vel_world_envs
        # update standing envs
        self.is_standing_env[env_ids] = r.uniform_(0.0, 1.0) <= self.cfg.rel_standing_envs
    ...
    def _update_command(self):
        """
            Post-processes the velocity command.
        """
        # the envs follow world frame velocity command
        world_frame_ids = self.is_vel_world_env.nonzero(as_tuple=False).flatten()

        # update robot frame velocity command
        # self.vel_command_b[robot_frame_ids] = self.vel_command_b[robot_frame_ids]
        # pass ...
        # update world frame velocity command -> robot frame velocity command
        self.vel_command_b[world_frame_ids] = math_utils.quat_apply_yaw(
            math_utils.quat_inv(self.robot.data.root_quat_w[[world_frame_ids]]), 
            self.vel_command_w[[world_frame_ids]]
        )

        # update robot frame heading command [固定heading朝向，全向运动]
        heading_error = math_utils.wrap_to_pi(self.heading_target - self.robot.data.heading_w)
        # update world frame heading command -> robot frame heading command [调整heading朝向，前向运动]
        heading_error[world_frame_ids] = math_utils.wrap_to_pi(
            torch.atan2(self.vel_command_b[world_frame_ids, 1], self.vel_command_b[world_frame_ids, 0])
        )
        # convert heading command
