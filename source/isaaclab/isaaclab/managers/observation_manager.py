...
class ObservationManager(ManagerBase):
    ...
    def compute_group(self, group_name: str, update_history: bool = False) -> torch.Tensor | dict[str, torch.Tensor]:
        ...
        for term_name, term_cfg in obs_terms:
            ...
            # set history buffer reference for delay-enabled noise models before applying noise
            if isinstance(term_cfg.noise, noise.NoiseModelCfg) and term_cfg.noise.func is not None:
                if hasattr(term_cfg.noise.func, 'set_history_buffer'):
                    if term_name in self._group_obs_term_history_buffer[group_name]:
                        term_cfg.noise.func.set_history_buffer(
                            self._group_obs_term_history_buffer[group_name][term_name]
                        )
                obs = term_cfg.noise.func(obs)
            elif isinstance(term_cfg.noise, noise.NoiseCfg):
        ...
        for group_name, group_cfg in group_cfg_items:
            ...
            for term_name, term_cfg in group_cfg_items:
                ...
                if term_cfg.noise is not None and isinstance(term_cfg.noise, noise.NoiseModelCfg):
                    ...
                    # check if this noise model has delay enabled and ensure history length is sufficient
                    if hasattr(term_cfg.noise, 'enable_delay') and term_cfg.noise.enable_delay:
                        max_delay = max(term_cfg.noise.delay_range)
                        term_cfg.history_length = max(term_cfg.history_length, max_delay)
